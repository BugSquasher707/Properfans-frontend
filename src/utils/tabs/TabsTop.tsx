...
const TabsTop = ({ link, tabs, type }: { link?: boolean; tabs: TabInterface[]; type: any }) => {
  ...
  return (
    <>
      <div className="flex w-full items-center justify-center space-x-[10px] border-t-1 border-grey-10">
        {tabs.map((tab: TabInterface, key: number) => (
          <button
            key={key}
            className={`-mt-1 flex items-center justify-center space-x-[6px] border-t-2 p-14 text-14 font-bold sm:p-20 ${
